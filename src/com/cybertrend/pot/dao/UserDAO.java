...
public class UserDAO {
	...
	public static void save(User user) throws SQLException{
		Connection conn = DatabaseService.getConnection();
		String sql = "INSERT INTO user (username, password, fullName, address1, address2, address3, zip, phone, email) VALUES (?,?,?,?,?,?,?,?,?)";
		PreparedStatement prep = conn.prepareStatement(sql);
		prep.setString(1, user.getUsername());
		prep.setString(2, user.getPassword());
		prep.setString(3, user.getFullName());
		prep.setString(4, user.getAddress1());
		prep.setString(5, user.getAddress2());
		prep.setString(6, user.getAddress3());
		prep.setString(7, user.getZip());
		prep.setString(8, user.getPhone());
		prep.setString(9, user.getEmail());
		prep.executeUpdate();
		conn.close();
	}
